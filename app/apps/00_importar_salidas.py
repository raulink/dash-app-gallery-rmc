...
app.layout = dbc.Container([
    dbc.Row([
        dbc.Col(html.H2("CONTROL DE SALIDAS - MI TELEFÉRICO", 
        ...
        dbc.Col([
            dbc.Card([
                dbc.CardBody([
                    ...
                    html.Div(id='status-msg', className="mt-2 text-muted", style={'fontSize': '12px'})
    ...
    dbc.Modal([
        ...
        dbc.ModalBody([
            dbc.ButtonGroup([
                dbc.Button("SELECCIONAR TODOS", id="select-all", color="dark", outline=True, size="sm"),
                dbc.Button("LIMPIAR", id="deselect-all", color="danger", outline=True, size="sm"),
            ], className="mb-3"),
        ...
        dbc.ModalFooter(
            dbc.Button("GUARDAR EN POSTGRESQL", id="btn-db-save", color="success")
        ),
    ], id="modal-selection", size="xl", is_open=False, backdrop="static"),
...
def limpiar_unicode(df):
    """
    Normaliza y limpia el DataFrame para asegurar compatibilidad con WIN1252.
    """
    def limpiar_texto(texto):
        if not isinstance(texto, str):
            return str(texto)
        
        # 1. Normalización de caracteres (combina caracteres acentuados)
        texto = unicodedata.normalize('NFKC', texto)
        
        # 2. Diccionario de reemplazos manuales para símbolos comunes que fallan
        reemplazos = {
            '\u2265': '>=', # ≥
            '\u2264': '<=', # ≤
            '\u2010': '-',  # Guiones especiales
            '\u2013': '-',
            '\u2014': '-',
            '\u02cb': '`',  # El carácter 0xcb 0x8b (acento grave)
            '\u00a0': ' ',  # Espacio en blanco especial
        }
        for original, sustituto in reemplazos.items():
            texto = texto.replace(original, sustituto)
        
        # 3. Forzar codificación WIN1252 ignorando caracteres que aún así no se puedan traducir
        # Esto evita que el programa falle si aparece un carácter nuevo desconocido
        temp_bytes = texto.encode('cp1252', errors='ignore')
        return temp_bytes.decode('cp1252')
...
def cargar_datos_inicio():
    try:
        query = "SELECT * FROM salidas ORDER BY id ASC"
...
# 2. Procesar Upload
@app.callback(
    [Output('modal-selection', 'is_open'),
...
def process_upload(contents, filename):
    if contents:
        try:
            ...
            if 'Código Salida' in df.columns:
                df = df[df['Código Salida'].notna()]
            #print (df.head())

            columnas_ui_nombres = ['Nº','Sistema','Código Salida','Fecha Salida','Código Solicitud','Fecha Solicitud','Tipo de Gasto','Partida Presupuestaria','Proveedor','Carnet Identidad','Nombres y Apellidos','Gerencia','Departamento','Cargo','Aclaración','Ubicación','Código Item','Código Anterior Item','Código Proveedor Item','Item','Unidad Medida','Grupo','Subgrupo','Cantidad','Precio Unitario IVA','Importe Calculado (Histórico)IVA']
            columnas_db = ['id','sistema','codigo_salida','fecha_salida','codigo_solicitud','fecha_solicitud','tipo','partida','proveedor','ci','nombre','gerencia','departamento','cargo','aclaracion','ubicacion','codigo','codigo_anterior','id_proveedor','item','unidad','grupo','subgrupo','cantidad','punitario','total']
...
# 3. Selección masiva
@app.callback(
    Output('modal-table', 'selected_rows'),
...
@app.callback(
    [Output('modal-selection', 'is_open', allow_duplicate=True),
     Output('main-table', 'data', allow_duplicate=True),
     Output('main-table', 'columns', allow_duplicate=True)],
    [Input('btn-db-save', 'n_clicks')],
    [State('modal-table', 'data'),
     State('modal-table', 'selected_rows')],
    prevent_initial_call=True
)
def save_db(n, table_data, selected_indices):
    if n and selected_indices:
        ...
        # Insertar en SQL
        selected_df.to_sql('salidas', con=engine, if_exists='append', index=False)
